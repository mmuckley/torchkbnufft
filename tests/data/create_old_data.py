...
def create_interp_data():
    ...
    test_params = [
        ([3, 2, 38, 2], 52, False),
        ([5, 3, 78, 96, 2], 410, False),
        ([1, 5, 17, 19, 12, 2], 112, False),
    ]
...
def create_nufft_data():
    torch.set_default_dtype(torch.double)
    test_params = [
        ([3, 1, 38, 2], 52, False),
        ([5, 1, 78, 96, 2], 410, False),
        ([1, 3, 17, 19, 12, 2], 112, False),
    ]
    ...
    with open("nufft_data.pkl", "wb") as f:
        pickle.dump(outputs, f)


def create_sense_nufft_data():
    torch.set_default_dtype(torch.double)
    test_params = [
        ([3, 2, 38, 2], 52, False),
        ([5, 3, 78, 96, 2], 410, False),
    ...
    for (shape, klength, is_complex) in test_params:
        ...
        forw_ob = tkbn.MriSenseNufft(im_size=im_size, smap=smap)
