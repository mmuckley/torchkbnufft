...
def test_complex_mult():
    x = np.random.normal(size=(8, 16, 10)) + 1j * \
        np.random.normal(size=(8, 16, 10))
    y = np.random.normal(size=(8, 16, 10)) + 1j * \
        np.random.normal(size=(8, 16, 10))
    x_torch = torch.stack(
        (torch.tensor(np.real(x)), torch.tensor(np.imag(x)))
    )
    y_torch = torch.stack(
        (torch.tensor(np.real(y)), torch.tensor(np.imag(y)))
    )
